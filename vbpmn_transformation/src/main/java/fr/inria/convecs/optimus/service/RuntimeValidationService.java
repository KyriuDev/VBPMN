...
public class RuntimeValidationService
{
									 ...
									 final FormDataMultiPart formData)
	{
		...
		try
		{
			...
			final File file1 = new File(outputDir + File.separator + fileInfo1.getFileName());
	...
	{
		final String scriptsPath = servletContext.getRealPath(SCRIPTS_PATH);
		final Response httpResponse;
		...
		try
		{
			...
			{
				final ModelValidator validator = new RuntimeValidator(scriptsPath, outputDir);
				final String result;
				...
				if (fileList.size() == 2)
				{
					...
					if ("property-and".equals(mode) || "property-implied".equals(mode))
					{
